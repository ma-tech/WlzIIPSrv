...
\hypersetup{
    ...
    pdffitwindow=false,
    pdfstartview={FitH},
    pdftitle={The Woolz Internet Image Protocol (IIP) Server},
    pdfauthor={Bill Hill},
...
\title{The Woolz Internet Image Protocol (IIP) Server}
\author{
...
\com{SEL} & Specify a component of a compound object to be displayed and its colour. See \ref{ssec:morphoexp}. & \texttt{SEL={\sltt E,R,G,B,A}}\\
...
or by a morphological expression which can combine
the components through the use of morphological
operators.
...
Morphological expressions are explained in \ref{ssec:morphoexp}.
...
\texttt{SEL={\sltt E,A}}\newline
\texttt{SEL={\sltt E} } \\
& \textbf{Input Parameters}& \texttt{UINT {\sltt E}} compound object index or morphological expression\newline
...
\subsection{Morphological Expressions}
\label{ssec:morphoexp}
Morphological expressions may be built using the components of
Woolz compound objects and basic morphological operators.
...
and
union.
Although domain and threshold are not strictly morphological operators
they have been included as they are useful to extract domains
from image values.
Morphological expressions are built from compound object
indices and the morphological operators. Two of the
morphological operators (intersection and union) may
given a list indices in their expressions.
The morphological expressions can be written using the
syntax defined in table \ref{tbl:morphosyntax}.
...
\end{description}
The morphological operators available and their parameters
are described in table \ref{tbl:morphoops}.
...
\emph{idx list}    	&:=& \verb=(=\emph{idx}\verb=|(=\emph{idx}-\verb=)|(=\emph{idx}-\emph{idx}\verb=)|(=-\emph{idx}\verb=))(=,\emph{idx list}\verb=)= \\
\emph{exp}         	&:=& \emph{idx}\verb=)|= \\
	    ...
	    &  & union\verb=(=\emph{exp list},\emph{exp list}\verb=) |= \\
\emph{idx}         	&:=& \verb=[0-9]+= \\
...
\hline
\end{tabular}
\caption{Syntax for morphological expressions.}
\label{tbl:morphosyntax}
\end{table}
...
union\verb=(=\emph{exp list}\verb=)=                                & The union of the domains in the given lists. \\
\hline
\end{tabular}
\caption{Descriptions of morphological operators}.
\label{tbl:morphoops}
\end{table}
...
Returns a jpeg tile 0 with the a quality factor 50\%.
...
\item[Sectioning, selection and morphological operations]
\begin{verbatim}
