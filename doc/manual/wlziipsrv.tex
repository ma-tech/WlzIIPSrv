...
\hypersetup{
    ...
    pdffitwindow=false,
    pdfstartview={FitH},
    pdftitle={The Woolz Internet Image Protocol (IIP) Server (Version 1.1.10)},
    pdfauthor={Bill Hill},
...
\title{The Woolz Internet Image Protocol (IIP) Server (Version 1.1.10)}
\author{
...
\hline
\com{CVT} 	 & Request an image to be returned as a composed image.
                   CVT accepts JPG, PNG and WLZ format requests.
		                    & \texttt{CVT={\sltt format}} \\
\com{DST}        & Specify the distance of the sectioning plane.
                                    & \texttt{DST={\sltt dis}} \\
\com{FPL}        & Specify a viewing section through fitting a plane to
                   the given points or compound indices.
                                    & \texttt{FPL={\sltt fspec[,fcpec]*}
				      \newline
				      where
				      \newline
				      {\sltt fspec = I|(X,Y,Z)}} \\
\com{FXP}        & Specify the fixed point of the viewing section rotation.
                                    & \texttt{FXP={\sltt X,Y,Z}} \\
\com{FXT}        & Specify the second fixed point of the viewing section
                   rotation.        & \texttt{FXT={\sltt X,Y,Z}} \\
\com{MAP}        & Define a colour or grey value mapping.
                                    & \texttt{MAP=
                                    \newline
...
\com{MOD}        & Specify the projection mode.
				    & \texttt{MOD={\sltt mode}} \\
\com{PAB}        & Specify the 3D query point absolute in the object
                   coordinate.      & \texttt{PAB={\sltt X,Y,Z}} \\
\com{PIT}        & Specify the pitch angle of the sectioning rotation.
				    & \texttt{PIT={\sltt angle}} \\
\com{PRL}        & Specify the 2D query point relative in tile or display or
                   tile coordinate. & \texttt{PRL={\sltt T,X,Y}} \\
\com{PTL}        & Retrieve a tile as a PNG image.
				    & \texttt{PTL={\sltt res,tile}} \\
\com{DPT}        & Specify the rendering depth.
				    & \texttt{DPT={\sltt depth}} \\
\com{RMD}        & Specify the rendering mode.
				    & \texttt{RMD={\sltt mode}} \\
\com{ROL}        & Specify the roll angle of the sectioning rotation.
				    & \texttt{ROL={\sltt angle}} \\
\com{SCL}        & Specify the scale used in the sectioning transformation.
				    & \texttt{SCL={\sltt scale}} \\
\com{SEL}        & Specify a component of a compound object to be displayed
                   and its colour. See \ref{ssec:imgpexp}.
				    & \texttt{SEL={\sltt E,R,G,B,A}} \\
\com{UPV}        & Specify the up vector for the \com{UP\_IS\_UP} mode.
				    & \texttt{UPV={\sltt X,Y,Z}} \\
\com{WLZ}        & Specify the Woolz object.
				    & \texttt{WLZ={\sltt path}} \\
\com{YAW}        & Specify the yaw angle of the sectioning rotation.
                                    & \texttt{YAW={\sltt angle}} \\
\hline
\end{tabular}
...
\com{Wlz-volume}            & The volume of the object or first selection
                              (if it exists). \\
...
\com{Wlz-sectioning-angles} & The pitch, yaw and roll angles of of the
                              sectioning plane. \\
\com{Wlz-coordinate-3D}     & The 3D coordinates defined in 2D by the
                              \com{PRL} command. \\
\com{Wlz-grey-stats}        & Simple statistics of the image values of the
                              object or first selection (if it exists). \\
\com{Wlz-grey-value}        & The grey or RGB value of a point specified either
                              the \com{PRL} or the \com{PAB} commands. \\
\com{Wlz-transformed-coordinate-3d} & The display coordinates and displacement
                              from the sectioning plane of a 3D point defined
			      by \com{PAB}. \\
\com{Wlz-foreground-objects}& The components of the compound object that 2D/3D
                              query point is a foreground. \\
\hline
\end{tabular}
...
Specify the Woolz object. It is similar to the \com{FIF} command, however
instead of loading Pyramidal Tiled TIFF images, it specifies a Woolz object.
Compered with \com{FIF}, the Woolz object is not loaded until a later
operation requires this. The Woolz objects are cached.
3D domain and value objects, and compound objects with 3D domain or value
components are supported. For a compound object the domain of the first
object must include all other object domains.
If file name ends with \texttt{.gz}, gzipped Woolz object is expected. \\
& \textbf{Syntax} & \texttt{WLZ={\sltt path}} \\
& \textbf{Input Parameters}&
  \texttt{PATH {\sltt path}} Full path of the Woolz object. \\
& \textbf{Response} &
  \com{WLZ} returns nothing when accompanied by other commands\\
...
\hrule\noindent
\begin{tabular}{p{\commandcolumna}p{\commandcolumnb}p{\commandcolumnc}}
\com{FPL} & \textbf{Purpose} & 
Specify the view by fitting a plane to given points or through components of
a compound object. \\
& \textbf{Syntax} & \texttt{FPL={\sltt fspec[,fcpec]*}
		    \newline
		    where
		    \newline
		    {\sltt fspec = I|(X,Y,Z)}} \\
& \textbf{Input Parameters}&
  \texttt{UINT  {\sltt I}} \newline Index of a compound object component
                           \newline 
  \texttt{FLOAT {\sltt X}} \newline Point x coordinate\newline 
  \texttt{FLOAT {\sltt Y}} \newline Point y coordinate\newline
  \texttt{FLOAT {\sltt Z}} \newline Point z coordinate \\
& \textbf{Response} & none \\
& \textbf{Example} & \outparam
                     \texttt{FXP=(120,20,15),(567,89,45),(339,87,62)} \\
& \textbf{Default value} &
  \texttt{Plane through (0,0,0) with PIT=0.0 and YAW=0.0} \\
\end{tabular}
\hrule\noindent
\begin{tabular}{p{\commandcolumna}p{\commandcolumnb}p{\commandcolumnc}}
...
\hrule\noindent
\begin{tabular}{p{\commandcolumna}p{\commandcolumnb}p{\commandcolumnc}}
\com{CVT} & \textbf{Purpose} & Request an image to be returned as a composed
image. CVT accepts JPG and PNG format requests.\\
...
\emph{exp}         	&:=& \emph{idx}\verb=|= \\
	    ...
	    &  & union\verb=(=\emph{exp list}
	              \verb=|=\emph{idx list},\emph{exp list}
		      \verb=|=\emph{idx list}\verb=)= \\
\emph{idx list}    	&:=& \verb=(=\emph{uint}
                             \verb=|=\emph{idx range}
			     \verb=|=\emph{idx list},\emph{idx list}\verb=)= \\
\emph{idx range}    	&:=& \emph{uint}\verb=-=\emph{uint} \\
\emph{idx}         	&:=& \emph{uint} \\
\emph{val}       	&:=& \verb=(=\emph{int}\verb=)|(=\emph{float}
                             \verb=)([eE]=\emph{int}\verb=)?= \\
\emph{real}             &:=& \emph{int}\verb=[.]=\emph{uint} \\
\emph{int}         	&:=& \verb=-?=\emph{uint} \\
\emph{uint}         	&:=& \verb=[0-9]+= \\
